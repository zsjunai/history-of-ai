...
主体章节已基本完成（本纪 10 / 世家 31 / 列传 23 / 书 25 / 表 134 条事件）。后续章节扩展或新增方向：

1. 在 Issues 区检索已有讨论，避免重复工作
2. 留言认领或新开 Issue 描述构想
3. Fork 仓库，创建分支撰写
4. 完成后提交 Pull Request

英文版翻译尤其欢迎——目前 4/90 章节已翻译，剩余大量空白。
...
| `year` | ✅ | 年份（字符串） |
| `event` | ✅ | 事件描述（中文） |
| `link` | ✅（建议） | 关联页面路径，必须指向已存在的页面 |
| `type` | ✅ | `paper` / `product` / `company` / `policy` / `person` / `event` / `milestone` |
| `importance` | ✅ | `milestone`（每个时代 1–2 条）/ `major` / `minor` |
| `month` | ⚪ 可选 | 1–12，仅在能确认月份时填写 |
